...
ipcMain.handle("auth-list-users", () => {
  // auth.listUsers() renvoie [{username, createdAt}, ...] côté disque,
  // mais le contrat déclaré dans types.ts (ElectronAPI.authListUsers)
  // est Promise<string[]> — la page de login attend des chaînes et
  // plantait en essayant d'afficher l'objet brut (React error #31).
  return auth.listUsers(getDocumentsPath()).map((u) => u.username);
});
