...
import { getMonthKey, getMonthLabel, MONTH_NAMES } from "../../constants.ts";
...
export default function InvestmentDetailPage({
  ...
  return (
    <div className="flex flex-col gap-5">
      <button
        onClick={onBack}
        className="self-start mb-1 bg-transparent border border-border rounded-lg text-text-muted py-1.5 px-3.5 text-[0.83rem] cursor-pointer transition-colors hover:bg-surface hover:text-text"
      >
        ← Retour au portefeuille
      </button>

      {/* Hero */}
      <div className="flex gap-5 p-5 bg-surface border border-border rounded-2xl flex-wrap">
        <div className="flex gap-3.5 items-start flex-1 min-w-[200px]">
          <span className="text-4xl flex-shrink-0">{ti.icon}</span>
          <div>
                      ...
                      <DeleteX
                        onClick={() => {
                          const h = (inv.valueHistory ?? []).filter(
                            (p) =>
                              !(p.date === pt.date && p.value === pt.value),
                          );
                          u({
                            valueHistory: h,
          ...
          <Section>
            ...
            <div className="mt-2.5">
              {(inv.gains ?? []).length === 0 && !showAddGain ? (
                <EmptySection>
                  Aucun gain encaissé — cliquez sur "+ Encaisser" pour en
                  ajouter
                </EmptySection>
              ) : (
                <div className="flex flex-col gap-1.5">
