import { getMonthLabel, getMonthKey } from "../lib/constants";
...
export default function MonthPaginator({
  ...
  return (
    <div className="flex items-center gap-2.5 bg-surface-soft border-none border-border rounded-2xl py-1.5 px-3">
      <button
        onClick={() => onChangeMonth(shiftMonth(viewMonth, -1))}
        className="w-10 h-10 rounded-[10px] border-none border-border bg-surface text-text text-[1.6rem] leading-none cursor-pointer transition-all flex items-center justify-center font-sans hover:border-primary hover:bg-surface-light hover:text-primary hover:scale-[1.08]"
      >
        ‹
      </button>
      ...
      <button
        ...
        className="w-10 h-10 rounded-[10px] border-none border-border bg-surface text-text text-[1.6rem] leading-none cursor-pointer transition-all flex items-center justify-center font-sans
      ...
      {!isCurrent && (
        <button
          ...
          className="py-[7px] px-4 rounded-[10px] border-none border-primary bg-primary/10 text-primary text-[0.85rem] font-semibold cursor-pointer transition-all whitespace-nowrap hover:bg-primary hover:text-white"
        >
          Aujourd'hui
        </button>
      )}
