  ...
  return (
    <ModalOverlay onClose={onClose}>
      <ModalBox>
        ...
        <div className="flex bg-surface rounded-xl p-1 gap-1 mb-3">
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-lg text-[0.82rem] font-semibold cursor-pointer transition-all
              ${!monthOnly ? "bg-primary text-white" : "bg-transparent text-text-muted hover:text-text"}`}
          >
            🌐 Tous les mois
          </button>
          <button
        ...
        {/* Alerte thématique — reste bordée, c'est un signal */}
        {hasOverride && (
        ...
        {dataPath && (
          <div className="mt-3 py-2.5 px-3.5 bg-surface rounded-[10px] text-[0.78rem] text-text-muted break-all">
    ...
    <ModalOverlay onClose={onClose}>
      <ModalBox>
        ...
        <div className="bg-surface rounded-xl p-4 mb-4 flex flex-col gap-2.5">
          ...
          {/* Alertes thématiques — restent bordées */}
          {willCarry && (
