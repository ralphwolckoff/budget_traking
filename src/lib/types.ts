...
export function generateRecurringExpenses(
  ...
  for (const id of ids) {
    ...
    let cursor = r.lastGeneratedMonth ? nextKey(r.lastGeneratedMonth) : r.startMonth
...
export function resolveGoalProgress(goal: FinancialGoal, appData: AppData): GoalProgress {
  const linkedIds = new Set((goal.linkedInvestmentIds ?? []).map(String))
  const linkedInvestments = Object.values(appData.investments ?? {}).filter(inv => linkedIds.has(String(inv.id)))
 
  const currentAmount = linkedInvestments.reduce((s, inv) => {
    const invested = inv.amount + (inv.payments ?? []).reduce((a, p) => a + p.amount, 0)
  ...
  const monthsUntilTarget = Math.max(0, Math.round((target.getTime() - today.getTime()) / (1000 * 60 * 60 * 24 * 30.44)))
 
  const currentMonthly = appData.savings
 
  const requiredMonthly = monthsUntilTarget > 0 ? remaining / monthsUntilTarget : remaining > 0 ? remaining : 0
  const onTrack = remaining === 0 || currentMonthly >= requiredMonthly
  const gap = onTrack ? 0 : requiredMonthly - currentMonthly
 
  const projectedMonths = currentMonthly > 0 ? Math.ceil(remaining / currentMonthly) : (remaining === 0 ? 0 : null)
 
  const pctComplete = goal.targetAmount > 0 ? Math.min(100, (currentAmount / goal.targetAmount) * 100) : 0
 
  return { currentAmount, remaining, monthsUntilTarget, requiredMonthly, currentMonthly, onTrack, gap, projectedMonths, pctComplete }
}
