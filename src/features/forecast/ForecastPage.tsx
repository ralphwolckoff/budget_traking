...
import { CATEGORIES, getMonthLabel, getMonthKey } from "../../lib/constants";
import type { AppData, ForecastItem } from "../../lib/types";
import {
  Section,
  SectionTitle,
  EmptyState,
...
function AddItemForm({
  ...
  return (
    <div>
      ...
      {catBudget > 0 && (
        <div className="bg-surface rounded-lg py-3 px-3.5 mb-4">
...
function CategoryGroup({
  ...
  return (
    <div
      className={`rounded-2xl overflow-hidden transition-colors ${over ? "ring-1 ring-danger/50" : ""}`}
    >
      <div
      ...
      >
        ...
        {over && (
          <span className="text-[0.72rem] bg-danger/15 text-danger rounded-md py-0.5 px-1.5 font-semibold flex-shrink-0">
            ⚠️ Dépassé
          </span>
        )}
        ...
        <div className="bg-surface-soft flex flex-col">
          {items.map((item) => (
            <div
              ...
              className={`flex items-center gap-3 py-2.5 px-4 border-t border-border/50 transition-colors cursor-pointer hover:bg-surface-light
...
export default function ForecastPage({
  ...
  return (
    <div className="flex flex-col gap-5">
      {/* Sélecteur de mois — pilules plates, actif en bg-primary uni */}
        ...
        {upcomingMonths.map((mk) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 py-2.5 px-[22px] rounded-full text-[0.95rem] font-semibold cursor-pointer transition-colors
                ${active ? "bg-primary text-white" : "bg-surface-soft text-text-muted hover:text-text"}`}
            >
              {getMonthLabel(mk)}
              {isCurrent && (
                <span
      ...
      {/* Barre globale — plate */}
      {totalForecast > 0 && (
        <div className="bg-surface-soft rounded-2xl py-4 px-5">
      ...
      {/* Alertes — restent bordées, ce sont des signaux, pas des cartes */}
