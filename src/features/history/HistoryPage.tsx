...
import { Expense, MonthOverrides } from "../../lib/types";
...
function MonthCard({
  ...
  return (
    <div
      ...
      className="bg-surface-soft rounded-2xl p-5 cursor-pointer transition-all hover:-translate-y-1 hover:bg-surface"
    >
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        {[
          ...
          <div
            key={i}
            className="flex items-start gap-2.5 bg-surface-soft rounded-xl p-3.5"
          >
...
export default function HistoryPage({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Sélecteur de période — pilules plates */}
          ...
          {PERIODS.map((p) => {
            ...
            return (
              <button
                ...
                disabled={disabled}
                className={`flex flex-col items-center gap-0.5 py-2.5 px-4 rounded-xl cursor-pointer transition-all
                  ${
                    active
                      ? "bg-primary text-white"
                      : disabled
                        ? "opacity-40 cursor-not-allowed bg-surface-soft text-text-muted"
      ...
      {filteredKeys.length > 0 && (
        <div className="flex justify-center">
          <button
            ...
            className={`flex items-center gap-2 py-2 px-4 rounded-lg text-[0.85rem] font-semibold cursor-pointer transition-colors
              ${showGrid ? "text-primary bg-primary/10" : "bg-surface-soft text-text-muted hover:text-text"}`}
          >
            {showGrid
